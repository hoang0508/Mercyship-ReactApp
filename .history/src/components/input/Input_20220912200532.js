...
const Input = ({ className = "" }) => {
  ...
  const demo =
    dataAssest &&
    dataAssest.length > 0 &&
    dataAssest.map((item) => item?.node?.title);
  console.log("🚀 ~ file: Input.js ~ line 33 ~ Input ~ demo", demo);
  const demo2 = [];

  for (const item2 of demo) {
    console.log("🚀 ~ file: Input.js ~ line 40 ~ Input ~ item2", item2);
    if (item2.includes("h")) {
      demo2.push(item2);
    }
  }
